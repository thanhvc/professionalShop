# The Tools Used in `mo-shopclient`

## Introduction

`mo-shopclient` is standards-based, so it uses all the usual tools to manage
...
project makes use and how they fit in to the `mo-shopclient` picture.

## Git

[Git](http://git-scm.com/) is a distributed version control system.
`mo-shopclient` uses git to manage its codebase. While in theory you don't have
to use Git once you download `mo-shopclient`, this project makes the assumption
...
While `mo-shopclient` makes heavy use of Node.js behind the scenes, you as the
...
you first start with `mo-shopclient` you have to tell NPM to install the
...
Don't worry about knowing Node.js in order to use `mo-shopclient`; Grunt is
...
to get started with `mo-shopclient`. But for those curious or looking to go a
...
Grunt is the engine behind `mo-shopclient`. It's the magic that makes it move.
...
`mo-shopclient` comes with a `bower.json` file that looks something like this:
...
in `mo-shopclient` by default in there to see what I mean.
